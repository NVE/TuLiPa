...
struct ShrinkableHorizon{H<:Horizon,S} <: Horizon
    ...
    function ShrinkableHorizon(
        subhorizon::Horizon,
        startafter::Millisecond,
        shrinkatleast::Millisecond,
...
getendperiodfromduration(h::_SHorizons, d::Millisecond) =
...
getsubperiods(coarse::_SHorizons, fine::Horizon, coarse_t::Int) =
    getsubperiods(coarse.subhorizon, fine, coarse_t)
getsubperiods(coarse::Horizon, fine::_SHorizons, coarse_t::Int) =
    getsubperiods(coarse, fine.subhorizon, coarse_t)
getsubperiods(coarse::_SHorizons, fine::_SHorizons, coarse_t::Int) =
...
function getlightweightself(h::_SHorizons)
    return ShrinkableHorizon(getlightweightself(h.subhorizon), h.handler)
end
...
getchanges(::ShrinkableHorizon, subhorizon::SequentialHorizon) =
    Dict{String,Any}("periods" => subhorizon.periods.data)
...
setchanges!(::ShrinkableHorizon, subhorizon::SequentialHorizon, changes::Dict) =
    subhorizon.periods.data .= changes["periods"]
setchanges!(::ShrinkableHorizon, subhorizon::AdaptiveHorizon, changes::Dict) =
    setchanges!(subhorizon, changes)
...
function gethorizonshrinker(
    ...
    updates_shift = [HORIZON_NOSHIFT for t = 1:T]
    updates_must = [true for t = 1:T]
...
makeshrinkable!(periods::SequentialPeriods, shrinkperiod::Int) =
    _makeshrinkable!(periods.data, shrinkperiod)
...
function shrink!(
    handler::SequentialPeriodsShrinker,
    p::SequentialPeriods,
    change::Millisecond,
)
    ...
    L = maximum(
        handler.shrinkperiods[i] for
        (i, isupd) in enumerate(handler.shrinkperiods_isupdated) if isupd
    )
...
function reset_shift!(
    ...
    numshiftperiods =
        sumshrink.value / sum(handler.shrinkperiods_maxduration).value *
        length(handler.shrinkperiods)
    ...
    for t = (last(handler.shrinkperiods)-nsp+1):(handler.last_shiftperiod-nsp+1)
    ...
    handler.remaining_duration =
        sum(v - handler.minperiod for v in handler.shrinkperiods_maxduration)
    return
end
...
function gethorizonshifter(h::Union{SequentialHorizon,AdaptiveHorizon})
    T = getnumperiods(h)
    shift = [HORIZON_NOSHIFT for t = 1:T]
    must = [false for t = 1:T]
    SequentialPeriodsShifter(shift, must, nothing)
end

makeshrinkable!(h::SequentialHorizon, handler::SequentialHorizonShrinker) =
    makeshrinkable!(h.periods, handler.shrinker)
makeshrinkable!(h::AdaptiveHorizon, handler::AdaptiveHorizonShrinker) =
    makeshrinkable!(h.macro_periods, handler.shrinker)
...
mayshiftfrom(h::SequentialHorizon, handler::SequentialHorizonShrinker, t::Int) =
    _common_mayshiftfrom(h, handler.shrinker, t)
mayshiftfrom(h::SequentialHorizon, handler::SequentialHorizonShifter, t::Int) =
    _common_mayshiftfrom(h, handler.shifter, t)
mayshiftfrom(h::AdaptiveHorizon, handler::AdaptiveHorizonShrinker, t::Int) =
    _common_mayshiftfrom(h, handler.shrinker, t)
mayshiftfrom(h::AdaptiveHorizon, handler::AdaptiveHorizonShifter, t::Int) =
    _common_mayshiftfrom(h, handler.shifter, t)
...
mustupdate(h::SequentialHorizon, handler::SequentialHorizonShrinker, t::Int) =
    _common_mustupdate(h, handler.shrinker, t)
mustupdate(h::SequentialHorizon, handler::SequentialHorizonShifter, t::Int) =
    _common_mustupdate(h, handler.shifter, t)
mustupdate(h::AdaptiveHorizon, handler::AdaptiveHorizonShrinker, t::Int) =
    _common_mustupdate(h, handler.shrinker, t)
mustupdate(h::AdaptiveHorizon, handler::AdaptiveHorizonShifter, t::Int) =
    _common_mustupdate(h, handler.shifter, t)

_common_mustupdate(h::SequentialHorizon, shrinker_shifter, t) =
    shrinker_shifter.updates_must[t]
_common_mustupdate(h::AdaptiveHorizon, shrinker_shifter, t) =
    shrinker_shifter.updates_must[div(t - 1, h.num_block)+1]
...
update!(h::SequentialHorizon, handler::SequentialHorizonShifter, start::ProbTime) =
    _common_update_shiftable!(h, handler, start)
update!(h::AdaptiveHorizon, handler::AdaptiveHorizonShifter, start::ProbTime) =
    _common_update_shiftable!(h, handler, start)
...
function _common_update_shrinkable!(h, handler, p, start)
    ...
    if change <= s.remaining_duration
        shrink!(s, p, change)
    elseif (change == (s.remaining_duration + s.minperiod)) &&
           (s.last_shiftperiod != HORIZON_NOSHIFT)
