...
buildprob(::JuMPHiGHSMethod, modelobjects) = JuMP_Prob(
    modelobjects,
    Model(optimizer_with_attributes(HiGHS.Optimizer, "simplex_scale_strategy" => 4)),
)
...
buildprob(::JuMPTulipMPCMethod, modelobjects) = JuMP_Prob(
    modelobjects,
    Model(
        optimizer_with_attributes(
            Tulip.Optimizer,
            "IPM_Factory" => Tulip.Factory(Tulip.MPC),
        ),
    ),
) # "Threads" => 2, cholmods LDL factorisation
...
buildprob(::JuMPClarabelMethod, modelobjects) =
    JuMP_Prob(modelobjects, Model(Clarabel.Optimizer))
