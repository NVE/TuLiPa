    ...
    for arrow in getarrows(plant)
        if !isingoing(arrow)
            ...
            for flow in balanceflows[prodbalance]
                if split(getinstancename(getid(flow)), "r")[1] ==
                   split(getinstancename(getid(plant)), "u")[1] # only difference in instancename is "r" and "u"
...
function getlowerreservoirplant(plant::Flow, balanceflows::Dict, storages::Vector)
    for arrow in getarrows(plant)
        if isingoing(arrow) &&
           (getinstancename(getid(getcommodity(getbalance(arrow)))) == "Hydro")
...
function getlowerreservoirpump(pump::Flow, storages::Vector)
    for arrow in getarrows(pump)
        if !isingoing(arrow) &&
           (getinstancename(getid(getcommodity(getbalance(arrow)))) == "Hydro") # not power market arrow
    ...
    for plant in plants
        ...
        if upperstorage isa Storage
            if haskey(upperstorage.metadata, RESERVOIRCURVEKEY)
                ...
                startupperstorage =
                    getparamvalue(getub(upperstorage), t, dummydelta) * startstorage / 100
                ...
                if lowerstorage isa Storage
                    ...
                    if haskey(lowerstorage.metadata, RESERVOIRCURVEKEY)
                        ...
                        startlowerstorage =
                            getparamvalue(getub(lowerstorage), t, dummydelta) *
                            startstorage / 100
                ...
                for arrow in getarrows(plant)
                    if getinstancename(getid(getcommodity(getbalance(arrow)))) == "Power"
                        if arrow isa BaseArrow # if standard arrow
                            arrow.conversion.param = StatefulParam(
                                TwoProductParam(
                                    arrow.conversion.param,
                                    ConstantParam(factor),
                                ),
                            )
                        elseif arrow isa SegmentedArrow # if pq-curve, adjust all points
                            for conversion in arrow.conversions
                                conversion.param = StatefulParam(
                                    TwoProductParam(
                                        conversion.param,
                                        ConstantParam(factor),
                                    ),
            ...
            if haskey(upperstorage.metadata, RESERVOIRCURVEKEY)
                ...
                if lowerstorage isa Storage
                    ...
                    if haskey(lowerstorage.metadata, RESERVOIRCURVEKEY)
                        rescurve = lowerstorage.metadata[RESERVOIRCURVEKEY]
                        startlowerstorage =
                            startstates[getinstancename(getid(lowerstorage))]
                        lowerheight = yvalue(rescurve, startlowerstorage)
                        # Make sure lower height is higher than outlet level
                        lowerheight = max(lowerheight, outletlevel)
                    else
                    ...
                    if getinstancename(getid(getcommodity(getbalance(arrow)))) == "Power"
                        if arrow isa BaseArrow # if standard arrow
                            if init
                                ...
                                arrow.conversion.param = StatefulParam(
                                    TwoProductParam(
                                        arrow.conversion.param.param.param1,
                                        ConstantParam(factor),
                                    ),
                                )
                            end
    ...
    for pump in pumps
        for arrow in getarrows(pump)
            ...
            if conversion isa PumpConversion
                ...
                lowerstorage isa Storage ||
                    error(string("No lower storage found for ", pumpname))
                haskey(lowerstorage.metadata, RESERVOIRCURVEKEY) || error(
                    "No reservoir curve for lower pump storage ",
                    getinstancename(getid(lowerstorage)),
                )
                ...
                upperstorage isa Storage ||
                    error(string("No upper storage found for ", pumpname))
                haskey(upperstorage.metadata, RESERVOIRCURVEKEY) || error(
                    "No reservoir curve for upper pump storage ",
                    getinstancename(getid(upperstorage)),
                )
...
function updateheadlosscosts!(
    method::ReservoirCurveSlopeMethod,
    clearing::Prob,
    masters::Vector,
    t::ProbTime,
)
        ...
        for obj in getobjects(master)
            if obj isa Storage
                if haskey(obj.metadata, RESERVOIRCURVEKEY)
                    (resid, headlosscost, T) = get_headlosscost_data_obj(
                        method,
                        master,
                        t,
                        dummydelta,
...
function updateheadlosscosts!(method::ReservoirCurveSlopeMethod, master::Prob, t::ProbTime)
    ...
    for obj in getobjects(master)
        if obj isa Storage
            if haskey(obj.metadata, RESERVOIRCURVEKEY)
                (resid, headlosscost, T) =
                    get_headlosscost_data_obj(method, master, t, dummydelta, reffactor, obj)
...
function get_headlosscost_data_obj(
    method::ReservoirCurveSlopeMethod,
    master::Prob,
    t::ProbTime,
    dummydelta::TimeDelta,
    reffactor::Float64,
    obj::Any,
)
    resid = getid(obj)
