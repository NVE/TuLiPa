...
struct PrognosisSeriesParam{L<:TimeVector,P<:TimeVector,Prog<:TimeVector,C<:TimeVector} <:
       Param
    level::L
    profile::P
    prognosis::Prog
    ...
    function PrognosisSeriesParam(level, profile, prognosis, confidencesteps::Int) # TODO: Add interface for confidencevector
        ...
        for i = 0:confidencesteps
            index[i+1] =
                first(prognosis.index) +
                Millisecond(round(Int, confidencedelta.value * (i - 1) / confidencesteps))
            values[i+1] = round((confidencesteps - i) / confidencesteps, digits = 3)
        end
        confidence = InfiniteTimeVector(index, values)

        new{typeof(level),typeof(profile),typeof(prognosis),typeof(confidence)}(
            level,
            profile,
            prognosis,
            confidence,
...
struct DynamicPrognosisSeriesParam{
    L<:TimeVector,
    P<:TimeVector,
    Prog<:TimeVector,
    C<:TimeVector,
...
isdurational(param::TwoProductParam) =
    isdurational(param.param1) || isdurational(param.param2)
...
isdurational(param::ExogenCostParam) =
    isdurational(param.price) || isdurational(param.conversion) || isdurational(param.loss)
isdurational(param::ExogenIncomeParam) =
    isdurational(param.price) || isdurational(param.conversion) || isdurational(param.loss)
isdurational(param::InConversionLossParam) =
    isdurational(param.conversion) || isdurational(param.loss)
isdurational(param::OutConversionLossParam) =
    isdurational(param.conversion) || isdurational(param.loss)
...
getparamvalue(param::StatefulParam, start::ProbTime, d::TimeDelta) =
    getparamvalue(param.param, start, d)
getparamvalue(param::FlipSignParam, start::ProbTime, d::TimeDelta) =
    -getparamvalue(param.param, start, d)
getparamvalue(param::ExogenIncomeParam, start::ProbTime, d::TimeDelta; ix = 0) =
    getparamvalue(param.price, start, d; ix) *
    getparamvalue(param.conversion, start, d) *
    (1 - getparamvalue(param.loss, start, d))
getparamvalue(param::ExogenCostParam, start::ProbTime, d::TimeDelta; ix = 0) =
    getparamvalue(param.price, start, d; ix) * getparamvalue(param.conversion, start, d) /
    (1 - getparamvalue(param.loss, start, d))
getparamvalue(param::InConversionLossParam, start::ProbTime, d::TimeDelta) =
    getparamvalue(param.conversion, start, d) * (1 - getparamvalue(param.loss, start, d))
getparamvalue(param::OutConversionLossParam, start::ProbTime, d::TimeDelta) =
    getparamvalue(param.conversion, start, d) / (1 - getparamvalue(param.loss, start, d))
getparamvalue(param::TransmissionLossRHSParam, t::ProbTime, d::TimeDelta) =
    getparamvalue(param.capacity, t, d) * param.loss * param.utilisation
...
function getparamvalue(param::TwoProductParam, start::ProbTime, d::TimeDelta; ix = 0)
    if ix == 0
        ...
        return getparamvalue(param.param1, start, d; ix) *
               getparamvalue(param.param2, start, d)
    end
end
...
function _prognosislogic(
    param::Union{PrognosisSeriesParam,DynamicPrognosisSeriesParam},
    datatime::DateTime,
    scenariotime::DateTime,
    d::TimeDelta,
    confidence::Float64,
    last_prognosis_time::DateTime,
)
    ...
    elseif (confidence == 1.0) && (datatime + getduration(d) <= last_prognosis_time) # Only use prognosis
        ...
        if datatime + getduration(d) <= last_prognosis_time # Combine prognosis and profile at a confidence (weighting)
            ...
            fullyprofilepart = getweightedaverage(
                param.profile,
                scenariotime + getduration(new_delta),
                d - new_delta,
            )
            ...
            profile =
                profilepart * (1 - confidence) +
                prognosispart * confidence +
                fullyprofilepart
        end
    end
    return profile
...
function getparamvalue(
    param::FossilMCParam,
    start::Union{PhaseinTwoTime,PhaseinFixedDataTwoTime,PhaseinPrognosisTime},
    d::TimeDelta,
)
...
function _umm_logic(param, start, d, new_profile_start, new_start_delta)
    ...
    if over > Millisecond(0)
        ...
        umm_part =
            getweightedaverage(param.ummprofile, new_umm_start, MsTimeDelta(new_umm_delta))
        profile_part = _phaseinLogic(
            start.phaseinvector,
            param,
            MsTimeDelta(new_profile_delta),
            new_scen_1,
            new_scen_2,
        )
...
function getparamvalue(
    param::UMMSeriesParam,
    start::Union{PhaseinTwoTime,PhaseinPrognosisTime},
    d::TimeDelta,
)
    datatime = getdatatime(start)
    scenariotime = getscenariotime(start)
    new_profile_start, new_start_delta =
        _get_new_profile_start(param, datatime, scenariotime)
    level_mw = getweightedaverage(param.level, datatime, d)
    if scenariotime > new_profile_start

        profile = _phaseinLogic(
            start.phaseinvector,
            param,
            d,
            start.scenariotime1,
...
function getparamvalue(param::UMMSeriesParam, start::ProbTime, d::TimeDelta)
    ...
    if over > Millisecond(0)
        ...
        umm_part =
            getweightedaverage(param.ummprofile, new_umm_start, MsTimeDelta(new_umm_delta))
        profile_part = getweightedaverage(
            param.profile,
            new_profile_start,
            MsTimeDelta(new_profile_delta),
        )
...
function includeM3SToMM3SeriesParam!(
    ::Dict,
    lowlevel::Dict,
    elkey::ElementKey,
    value::M3SToMM3SeriesParam,
)
    checkkey(lowlevel, elkey)
    deps = Id[]
...
function includeMWToGWhSeriesParam!(
    ::Dict,
    lowlevel::Dict,
    elkey::ElementKey,
    value::MWToGWhSeriesParam,
)
    checkkey(lowlevel, elkey)
    deps = Id[]
    lowlevel[getobjkey(elkey)] = value
...
function includeCostPerMWToGWhParam!(
    ::Dict,
    lowlevel::Dict,
    elkey::ElementKey,
    value::CostPerMWToGWhParam,
)
    checkkey(lowlevel, elkey)
    deps = Id[]
    lowlevel[getobjkey(elkey)] = value
...
function includeMeanSeriesParam!(
    ::Dict,
    lowlevel::Dict,
    elkey::ElementKey,
    value::MeanSeriesParam,
)
    checkkey(lowlevel, elkey)
    deps = Id[]
    lowlevel[getobjkey(elkey)] = value
...
function includeMeanSeriesIgnorePhaseinParam!(
    ::Dict,
    lowlevel::Dict,
    elkey::ElementKey,
    value::Dict,
...
function includeMeanSeriesIgnorePhaseinParam!(
    ...
    lowlevel::Dict,
    elkey::ElementKey,
    value::MeanSeriesIgnorePhaseinParam,
)
    checkkey(lowlevel, elkey)
    deps = Id[]
    lowlevel[getobjkey(elkey)] = value
...
function includePrognosisSeriesParam!(
    ::Dict,
    lowlevel::Dict,
    elkey::ElementKey,
    value::Dict,
...
INCLUDEELEMENT[TypeKey(PARAM_CONCEPT, "MeanSeriesIgnorePhaseinParam")] =
    includeMeanSeriesIgnorePhaseinParam!
INCLUDEELEMENT[TypeKey(PARAM_CONCEPT, "PrognosisSeriesParam")] =
