    ...
    if isexogen(arrow.balance)
        ...
        if arrow.isingoing
            ...
            elseif isnothing(arrow.loss)
                ...
                param =
                    ExogenIncomeParam(getprice(arrow.balance), arrow.conversion, arrow.loss)
            end
        else
            if isnothing(arrow.loss) && isone(arrow.conversion)
                param = getprice(arrow.balance)
            elseif isnothing(arrow.loss)
                ...
                param =
                    ExogenCostParam(getprice(arrow.balance), arrow.conversion, arrow.loss)
            end
        end
...
function setconstants!(p::Prob, var::Any, arrow::BaseArrow)
    ...
    if !isexogen(arrow.balance)
        ...
        if !_must_dynamic_update(param)
            ...
            for s = 1:getnumperiods(balancehorizon)
...
function includeSegmentedArrow!(
    toplevel::Dict,
    lowlevel::Dict,
    elkey::ElementKey,
    value::Dict,
    ...
    (id, conversionparams, ok) =
        getdictparamlist(lowlevel, elkey, value, CONVERSION_CONCEPT)
    all_ok = all_ok && ok
    _update_deps(deps, id, ok)
