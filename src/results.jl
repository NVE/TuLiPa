...
function order_result_objects(resultobjects, includeexogenprice = true)
    ...
    for obj in resultobjects
        ...
        if obj isa BaseFlow
            ...
            powerarrowbool = [
                (getid(getcommodity(getbalance(arrow))) == Id("Commodity", "Power")) &
                !(arrow isa SegmentedArrow) for arrow in arrows
            ]
...
function get_results!(
    ...
    for (j, jj) in enumerate(powerrange)
        ...
        for i = 1:length(plants) # TODO: Balance and variable can have different horizons
                ...
                if sum(pqarrowbool) == 1
                    ...
                    for (k, conversion) in enumerate(getconversions(arrow))
                        ...
                        if isexogen(getbalance(arrow))
                            ...
                            querydelta = gettimedelta(horizon, j)
                            conversionvalue =
                                getparamvalue(conversion, querystart, querydelta)
                            production[jj, i] +=
                                getvarvalue(problem, segmentid, j) * conversionvalue
                        else
                            production[jj, i] +=
                                getvarvalue(problem, segmentid, j) *
                                abs(getconcoeff(problem, plantbalances[i], segmentid, j, j))
                        end
                    end
                else
                    if isexogen(modelobjects[plantbalances[i]])
                        ...
                        querydelta = gettimedelta(horizon, j)
                        conversionvalue =
                            getparamvalue(conversionparam, querytime, querydelta)
                        production[jj, i] =
                            getvarvalue(problem, plants[i], j) * conversionvalue
                    else
                        production[jj, i] =
                            getvarvalue(problem, plants[i], j) *
                            abs(getconcoeff(problem, plantbalances[i], plants[i], j, j))
                    end
                end
            else
                production[jj, i] =
        ...
        for i = 1:length(demands) # TODO: Balance and variable can have different horizons
            if getconceptname(demands[i]) != DEMAND_CONCEPT
                if isexogen(modelobjects[demandbalances[i]])
                    ...
                    consumption[jj, i] =
                        getvarvalue(problem, demands[i], j) * conversionvalue
                else
                    consumption[jj, i] =
                        getvarvalue(problem, demands[i], j) *
                        abs(getconcoeff(problem, demandbalances[i], demands[i], j, j))
                end
            else
        ...
        for i = 1:length(batterystorages)
...
function init_results(
    ...
    modelobjects,
    resultobjects,
    numperiods_powerhorizon,
    numperiods_hydrohorizon,
    periodduration_power,
    t,
    ...
    demandarrows,
    hydrostorages,
    batterystorages = order_result_objects(resultobjects, includeexogenprice)
    ...
    get_results!(
        ...
        batterystorages,
        modelobjects,
        1:numperiods_powerhorizon,
        1:numperiods_hydrohorizon,
        periodduration_power,
        t,
    )
...
function update_results!(
    ...
    powerrange =
        Int(numperiods_powerhorizon * (step - 1) + 1):Int(numperiods_powerhorizon * (step))
    hydrorange =
        Int(numperiods_hydrohorizon * (step - 1) + 1):Int(numperiods_hydrohorizon * (step))
    get_results!(
        problem,
        prices,
        rhstermvalues,
                ...
                if obj isa Balance
                    ...
                    if commodity in keys(resultinfo[key])
                        for rhsterm in getrhsterms(obj)
                            ...
                            if any(
                                key -> occursin(key, rhsterminstancename),
                                resultinfo[key][commodity],
                            )
            ...
            for obj in resultobjects
                if obj isa BaseFlow
                    for commodity in keys(resultinfo[key])
                        ...
                        if any(
                            key -> occursin(key, varinstancename),
                            resultinfo[key][commodity],
                        )
            ...
            if key == "RHSTerms"
                ...
                for i in eachindex(rhsterms)
                    for (j, jj) in enumerate(periodrange)
                        othervalues[key][commodity][jj, i] =
                            getrhsterm(problem, balances[i], rhsterms[i], j)
                    end
                end
            elseif key == "Vars"
