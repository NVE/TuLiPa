...
function addhydro!(
    ...
    storagecap::Union{Real,Nothing} = nothing,
) # Mm3
    hydrobalance = name
    ...
    for fuel in ["Coal", "Gas"]
        ...
        push!(
            ...
            DataElement(
                ...
                name,
                Dict(
                    "FuelProfile" => "Profile" * fuel,
                    "FuelLevel" => "Level" * fuel,
                    "CO2Factor" => "CO2Factor" * fuel,
                    "CO2Profile" => "CO2Profile",
                    "CO2Level" => "CO2Level",
                    "Efficiency" => "Efficiency" * fuel,
                    "VOC" => "VOC" * fuel,
                ),
            ),
        )
...
function gettestlevels()
    ...
    push!(
        ...
        DataElement(
            ...
            "VectorTimeValues",
            "LevelDemandGER",
            Dict("Vector" => Float64[50000, 55000]),
        ),
    ) # MW
    push!(
        levels,
        DataElement(
            ...
            "VectorTimeValues",
            "LevelWindGER",
            Dict("Vector" => Float64[30000, 40000]),
        ),
    ) # MW
    push!(
        levels,
        DataElement(
            ...
            "VectorTimeValues",
            "LevelSolarGER",
            Dict("Vector" => Float64[60000, 80000]),
        ),
    ) # MW
    push!(
        levels,
        DataElement(
            ...
            "VectorTimeValues",
            "LevelGas",
            Dict("Vector" => Float64[90000, 50000]),
        ),
    ) # €/GWh
    push!(
        levels,
    ...
    for name in [
        ...
        push!(
            ...
            DataElement(
                ...
                name,
                Dict(
                    TIMEINDEX_CONCEPT => "DataLevelsTimeIndex",
                    TIMEVALUES_CONCEPT => name,
                ),
            ),
        )
    end
...
function gettestprofiles()
    ...
    datetimes = DateTime[getisoyearstart(1981) + Hour((i - 1) * 1113) for i = 1:8]
    ...
    push!(
        ...
        DataElement(
            ...
            "ProfileGas",
            Dict(
                TIMEVALUES_CONCEPT => "ValuesProfileGas",
                TIMEINDEX_CONCEPT => "IndexProfileGas",
            ),
        ),
    )
    return elements
