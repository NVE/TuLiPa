module TestTimeOffset

using Dates, TuLiPa, Test
...
function run_tests()
    ...
    prob_time =
        PhaseinFixedDataTwoTime(dt1, dt2, dt2, phaseinoffset, phaseindelta, phaseinsteps)
    td_offset = TimeDeltaOffset(td)
    offset_time = getoffsettime(prob_time, td_offset)
    @test offset_time.datatime == DateTime("2021-01-01T00:00:00")
    @test offset_time.scenariotime1 == DateTime("2021-01-11T00:00:00")
    ...
    vals = Array([sin(x) + 1 for x = 1:17641])
    vals[1:8820] .*= 1.2
    level = InfiniteTimeVector{Vector{DateTime},Vector{Float64}}(
        [DateTime("2021-01-04T00:00:00"), DateTime("2024-12-30T00:00:00")],
        [100.0, 100.0],
    )
    index =
        DateTime("1980-12-29T00:00:00"):Millisecond(3600000):DateTime("1983-01-03T00:00:00")
    profile = RotatingTimeVector(
        index,
        vals,
        DateTime("1980-12-29T00:00:00"),
        DateTime("1983-01-03T00:00:00"),
    )
    param = MWToGWhSeriesParam(level, profile)
    rhsterm = BaseRHSTerm(Id("RHSTerm", "WindGER_test"), param, true)

    function get_phaseintwotime_offset_value_samples(
        offset,
        scenario_dt1,
        scenario_dt2,
        rhsterm,
    ...
    IsoYearOffset_value_1, IsoYearOffset_value_15, IsoYearOffset_value_25 =
        get_phaseintwotime_offset_value_samples(
            offset,
    ...
    ScenarioOffset_value_1, ScenarioOffset_value_15, ScenarioOffset_value_25 =
        get_phaseintwotime_offset_value_samples(
            offset,
            getisoyearstart(1981),
            getisoyearstart(1981),
    ...
    TimeDeltaOffset_value_1, TimeDeltaOffset_value_15, TimeDeltaOffset_value_25 =
        get_phaseintwotime_offset_value_samples(
            offset,
            getisoyearstart(1981),
            getisoyearstart(1982),
            rhsterm,
        )
